...
Uses RandomForestRegressor trained on real monthly climate normals from
meteorological records (see data_collector.py). Falls back to synthetic
data if real_climate_data.csv has not been generated yet.
"""
import os
import pickle
import numpy as np
...
REAL_DATA_PATH = os.path.join(os.path.dirname(__file__), "real_climate_data.csv")


def load_real_dataset() -> pd.DataFrame:
    """
    Load the real climate dataset generated by data_collector.py.
    Raises FileNotFoundError if the CSV has not been generated yet.
    """
    from data_collector import load_dataset
    return load_dataset(REAL_DATA_PATH)
...
def train_model(df: pd.DataFrame = None) -> tuple:
    ...
    Data source priority:
      1. Caller-supplied DataFrame (df argument)
      2. real_climate_data.csv — real monthly climate normals for ~45 global
         cities (generated by data_collector.py)
      3. Synthetic fallback (1 000 random samples) when the CSV is absent
    ...
    if df is None:
        if os.path.exists(REAL_DATA_PATH):
            ...
            print(f"Using real climate data: {len(df)} samples from {df['city'].nunique()} cities")
        else:
            print("real_climate_data.csv not found — falling back to synthetic data.")
            print("Run 'python data_collector.py' to generate the real dataset.")
...
if __name__ == "__main__":
    # Auto-generate real dataset if not present
    if not os.path.exists(REAL_DATA_PATH):
        print("real_climate_data.csv not found — generating from climate normals...")
        from data_collector import build_real_dataset, save_dataset
        df_real = build_real_dataset()
        save_dataset(df_real, REAL_DATA_PATH)
    else:
        print(f"Using existing {REAL_DATA_PATH}")
